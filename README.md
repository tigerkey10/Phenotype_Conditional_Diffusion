...
### Orientation for readers from vision generative modeling

This is image-diffusion machinery applied to a non-image signal. The familiar parts transfer
verbatim; the unfamiliar parts are exactly the research problem.

| Image generation | This project (scRNA-seq) |
|------------------|--------------------------|
| an image (sample) | a **cell** (sample) |
| pixel vector `x ∈ ℝ^{H·W·C}` | **gene-expression vector** `x ∈ ℝ^G` (G = 1,000–2,000) |
| class label + classifier-free guidance | **cell-type label** + CFG (identical) |
| U-Net / CNN denoiser | **MLP** denoiser (no spatial grid — genes are an unordered set) |
| FID (Fréchet Inception Distance) | **MMD / 2-Wasserstein**, incl. the Fréchet/Gaussian-W2 = FID formula |
| t-SNE mode-coverage check | **UMAP** of real vs generated |
| SDEdit / image-to-image / attribute editing | **perturbation editing**: control → stimulated cell (§11) |

**Where it stops looking like images** (and why a literal DDPM fails): the data has **no
spatial structure** (MLP, not U-Net); its per-dimension marginal is a **spike at zero — ~93% of
every vector is exactly 0** (dropout), with a thin non-negative tail in [0, 7], *not* a dense
natural-image histogram; and standard DDPM's implicit unit-variance assumption is violated, so a
naive implementation explodes (generated values ~200× the real scale). Reproducing the exact
zeros needs an explicit **on/off gate** (spike-and-slab) that has no analogue in image diffusion.
§5 documents the five fixes; `DATA.md` §0.1 gives the full vision↔scRNA-seq mapping.
